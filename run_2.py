...
import time
import os
from model.scatter_gather.mamba_ssm_modeling import MambaForCausalLM_SSM
from model.scatter_gather.mamba_gate_modeling import MambaForCausalLM_Gate
...
def run_ssm(rank, world_size, devices_map, num_cycles):
    ...
    input_ids = torch.load("./input/input_ids_2.pt").to(device)
...
def run_gate(rank, world_size, devices_map, num_cycles):
    ...
    if rank == 0:
        ...
        torch.save(logits, "./output/run_2.pt")
...
if __name__ == "__main__":
    ...
    torch.save(input_ids, "./input/input_ids_2.pt")
