...
import time
import os
from model.send_recv.mamba_ssm_modeling import MambaForCausalLM_SSM
from model.send_recv.mamba_gate_modeling import MambaForCausalLM_Gate
...
def run_ssm(rank, world_size, devices_map, num_cycles):
    ...
    input_ids = torch.load("./input/input_ids_3.pt").to(device)
...
def run_gate(rank, world_size, devices_map, num_cycles):
    ...
    if rank == 0:
        ...
        torch.save(logits, "./output/run_3.pt")
...
if __name__ == "__main__":
    ...
    torch.save(input_ids, "./input/input_ids_3.pt")
