...
class MambaMixer(nn.Module):
    ...
    def __init__(self, config: MambaConfig, layer_idx: int):
        ...
        # Input projection (split by intermediate_size)
    ...
    # fmt: off
    def slow_forward(self, input_states, counter = None):
        batch_size, seq_len, _ = input_states.shape
        # torch.save(input_states, f"log/all_to_all_single/input_states/input_states_{self.layer_idx}.pt")

        # 1. Gated MLP's linear projection
        gate = self.in_proj(input_states).transpose(1, 2)  
        gate = self.act(gate)
        gate = gate.contiguous()
        # torch.save(gate, f"log/all_to_all_single/gate/gate_{self.layer_idx}.pt")
        ...
        if dist.is_initialized():
            ...
            # output_split_sizes = scan_output_splits
            scan_output_splits = []
            for rank in range(world_size):
                ...
                if rank in self.config.reshard_strategy:
                    ...
                    size = end - start
                scan_output_splits.append(size)

            # input = gate
            gate = gate.movedim(1,0)
            gate = gate.contiguous()

            # input_split_Sizes = gate_split_sizes
            gate_split_sizes = scan_output_splits.copy()
            ...
            # torch.save(scan_output, f"log/all_to_all_single/scan_output/scan_output_{self.layer_idx}.pt")
    ...
    def forward(
        ...
        return self.slow_forward(hidden_states, counter)
...
class MambaBlock(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        hidden_states = self.mixer(hidden_states, counter = counter)

        if self.config.rank == min(self.config.devices_map['gate'].keys()):
...
class MambaModel_Gate(MambaPreTrainedModel_Gate):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """ 
        # torch.save(input_ids, "log/all_to_all_single/input_ids.pt")
        ...
        for i, mixer_block in enumerate(self.layers):
            ...
            # torch.save(hidden_states, f"log/all_to_all_single/hidden/hidden_{i}.pt")

            
            if dist.is_initialized() and len(self.config.devices_map['gate']) > 1:
