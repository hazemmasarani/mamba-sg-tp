...
class MambaMixer(nn.Module):
    ...
    def __init__(self, config: MambaConfig, layer_idx: int):
        ...
        self.conv1d = nn.Conv1d(
            in_channels=self.local_intermediate_size,
            out_channels=self.local_intermediate_size,
            bias=config.use_conv_bias,
            kernel_size=config.conv_kernel,
            groups=self.local_intermediate_size,
        ...
        self.x_proj = nn.Linear(self.local_intermediate_size, self.time_step_rank + self.ssm_state_size * 2, bias=False)
        # time step projection (discretization)
        self.dt_proj = nn.Linear(self.time_step_rank, self.local_intermediate_size, bias=True)

        # S4D real initialization. These are not discretized!
        # The core is to load them, compute the discrete states, then write the updated state. Keeps the memory bounded
        A = torch.arange(1, self.ssm_state_size + 1, dtype=torch.float32)[None, :]
        A = A.expand(self.local_intermediate_size, -1).contiguous()

        self.A_log = nn.Parameter(torch.log(A))
        self.D = nn.Parameter(torch.ones(self.local_intermediate_size))
    ...
    def slow_forward(self, input_states, cache_params: MambaCache_SSM | None=None, cache_position:torch.LongTensor | None=None, attention_mask: torch.LongTensor | None = None, counter = None):
        ...
        # torch.save(input_states, f"log/all_to_all_single/input_states/input_states_{self.layer_idx}_{self.config.rank}.pt")

        # 1. Gated MLP's linear projection  
        hidden_states = self.in_proj(input_states).transpose(1, 2)      # [batch_size, Intermediate_size/world_size, seq_len]
        
        if self.in_proj.bias is not None:
            print(f"In projection bias is not None for layer {self.layer_idx}")

        if attention_mask is not None:
            hidden_states = hidden_states * attention_mask.unsqueeze(1)
        
        # torch.save(self.in_proj.weight, f"log/all_to_all_single/in_proj/in_proj_{self.layer_idx}_{self.config.rank}.pt")
        # torch.save(self.in_proj.bias, f"log/all_to_all_single/in_proj_bias/in_proj_{self.layer_idx}_{self.config.rank}.pt")
        ...
        if cache_params is not None:
            ...
            ssm_state = torch.zeros(
                (batch_size, self.local_intermediate_size, self.ssm_state_size),
                device=hidden_states.device, dtype=dtype
            )
            hidden_states = self.act(self.conv1d(hidden_states)[..., :seq_len])         # [batch, intermediate_size, seq_len]

        # torch.save(hidden_states, f"log/all_to_all_single/hidden/hidden_{self.layer_idx}_{self.config.rank}.pt")
        ...
        if dist.is_initialized() and len(self.config.devices_map['ssm']) > 1:
            dist.all_reduce(ssm_parameters, group=self.config.local_group)
            # torch.save(ssm_parameters, f"log/all_to_all_single/ssm_parameters/ssm_parameters_{self.layer_idx}.pt")
        ...
        if self.use_mambapy and self.training and cache_params is None:
            ...
            # torch.save(scan_output, f"log/all_to_all_single/scan_output/scan_out_{self.layer_idx}_{self.config.rank}.pt")
            ...
            # torch.save(scan_output, f"log/all_to_all_single/scan_output/scan_output_{self.layer_idx}_{self.config.rank}.pt")
        ...
        # torch.save(contextualized_states, f"log/all_to_all_single/contextualized_states/contextualized_states_{self.layer_idx}_{self.config.rank}.pt")
...
class MambaBlock(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        if self.config.rank == min(self.config.devices_map['ssm'].keys()):
...
class MambaPreTrainedModel_SSM(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, MambaMixer):
            ...
            A = A.expand(module.local_intermediate_size, -1).contiguous()
            ...
            dt = torch.exp(
                torch.rand(self.config.local_intermediate_size)
